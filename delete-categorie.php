...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $aboQuery = $db->prepare('DELETE *FROM membres WHERE id_categorie = id_categorie ');
        // Execution de la requête
        $aboQuery->execute(['id_categorie' => $id_categorie]);
        // Assignation de l'information à $abo
        ...
        if ($abo) {
            ...
            $deleteAboStmt = $db->prepare('DELETE FROM categorie WHERE id_categorie= id_categorie');
            // Execution de la requête
    ...
    header('location:' . 'categorie.php?type=' . $type . '&message=' . $message);
} else {
